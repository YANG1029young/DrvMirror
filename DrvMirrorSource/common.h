...
typedef struct
{
    ...
    int leftcut;                      //拼接中间图左边切断宽度
    int rightcut;                     //拼接中间图右边切断宽度
    int OptimizWidth;                 //拼缝优化宽度

    double ear_heightscale_background;  //背景图裁掉中间图的高度比例
    double ear_scale_side;              //两侧图缩小相对于中间图的比例
    double ear_startscale_background;   //从上开始，背景图的起始位置

    bool BlendSwitch;                   //是否开启拼接

    bool ObjDetc;                       //是否开启图像识别
